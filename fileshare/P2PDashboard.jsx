...
export default function P2PDashboard({
  ...
  const [planMode, setPlanMode] = useState('Standard'); // 'Standard', 'Free Trial', 'Premium'
  ...
  // Settings
  const [alwaysAwake, setAlwaysAwake] = useState(false);
  ...
  const inboxWsRef = useRef(null);
  const inboxPeersRef = useRef(new Map()); // senderId -> { pc, dc, pIce, activeFileId }
  const filesRef = useRef(new Map()); // fileId -> { id, name, size, mimeType, chunks, blob, status, etc. }
  const acceptedBatchesRef = useRef({});
  const wakeLockRef = useRef(null);
  const logTimerRef = useRef(null);
  const logSecondsRef = useRef(0);
  ...
  // Room status check
  const checkPlanStatus = (roomCode, isPerm) => {
    if (!isPerm) {
  ...
  const startInbox = (roomCode, authToken) => {
    startSessionTimer();
  ...
  // WebRTC Peer setup
  const setupPeer = (roomCode, senderId) => {
      ...
      channel.onmessage = e => {
        ...
        if (typeof d === 'string') {
          ...
          if (msg.type === 'meta') {
            ...
            const hasOPFS = typeof window !== 'undefined' && !!(navigator.storage && navigator.storage.getDirectory);
            ...
            const fileData = fileId ? filesRef.current.get(fileId) : null;
            if (fileData) {
          ...
          if (fileData) {
            ...
            const processChunk = async (chunkData) => {
              ...
              if (fileData.opfsWritable && !fileData.useMemoryFallback) {
                ...
                fileData.chunks.push(chunkData);
                fileData.receivedBytes = fileData.chunks.reduce((acc, c) => acc + c.byteLength, 0);
              }
  ...
  const updateFileProgress = (fileId, status, ratio, speed, eta, blob = null) => {
    setFilesList(prev =>
      prev.map(f => {
        if (f.id === fileId) {
  ...
  const downloadInboxFile = (fileId) => {
    const fileData = filesRef.current.get(fileId);
  ...
  const toggleSelectAllFiles = (checked) => {
    if (checked) {
  ...
  // Zip Generator logic
  const generateZipBlob = async (files) => {
    const crcTable = window.crcTable || (window.crcTable = (() => {
      const table = [];
      for (let n = 0; n < 256; n++) {
        let c = n;
        for (let k = 0; k < 8; k++) {
  ...
  const handleAlwaysAwake = async (checked) => {
            ...
            filesList.map(f => {
              ...
              return (
                <div key={f.id} style={{ background: 'rgba(255,255,255,0.01)', border: '1px solid var(--b)', borderRadius: 'var(--r)', padding: '0.85rem', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                  <div style={{ display: 'flex', justifyStyle: 'space-between', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.5rem' }}>
