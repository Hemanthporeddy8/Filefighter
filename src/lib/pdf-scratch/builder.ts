...
export async function buildPdfFromCanvasesScratch(canvases: HTMLCanvasElement[]): Promise<Blob> {
  ...
  // 2: Pages Parent Container
  // For each page index i:
  //   3 + i*3: Page Object definition
  //   4 + i*3: Contents Stream (drawing command)
  //   5 + i*3: XObject Image stream
  ...
  for (let i = 0; i < numPages; i++) {
    ...
    // Convert canvas image into high-resolution binary JPEG bytes
